...
This will give you the =HEAD= of the =master= branch which strives to always be in a working condition.  If you want a particular release of Worch you may indicate it by adding =@TAG= where =TAG= is the release tag.
...
#+BEGIN_EXAMPLE
  ...
  $ git clone git://github.com/brettviren/worch.git
  $ cd /path/to/work
  $ cp /path/to/worch/wscript .   # <-- customize if desired
  $ waf --prefix=/path/to/install \
        --orch-config=/path/to/worch/examples/simple/*.cfg \
        configure
